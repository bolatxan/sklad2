# Создаю исправленный HTML файл
html_content = '''<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Управление складом</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <div class="app">
        <div class="tab-content">
            <!-- Панель управления -->
            <div id="dashboard" class="tab-panel active">
                <div class="container">
                    <div class="section-header">
                        <h2>Панель управления</h2>
                    </div>
                    
                    <div class="metrics-grid">
                        <div class="metric-card">
                            <div class="metric-value" id="totalOrdersCount">0</div>
                            <div class="metric-label">Всего заказов</div>
                        </div>
                        <div class="metric-card">
                            <div class="metric-value" id="totalRevenue">0 ₸</div>
                            <div class="metric-label">Общая выручка</div>
                        </div>
                        <div class="metric-card">
                            <div class="metric-value" id="totalProfit">0 ₸</div>
                            <div class="metric-label">Прибыль</div>
                        </div>
                        <div class="metric-card">
                            <div class="metric-value" id="profitMargin">0%</div>
                            <div class="metric-label">Рентабельность</div>
                        </div>
                    </div>

                    <div class="recent-orders" id="recentOrders">
                        <h3>Последние заказы</h3>
                        <div id="recentOrdersList"></div>
                    </div>
                </div>
            </div>

            <!-- Заказы -->
            <div id="orders" class="tab-panel">
                <div class="container">
                    <div class="section-header">
                        <h2>Заказы</h2>
                        <button class="btn btn--primary add-order-btn" id="addOrderBtn">+ Новый заказ</button>
                    </div>
                    
                    <div class="filters">
                        <select class="form-control" id="statusFilter">
                            <option value="">Все статусы</option>
                            <option value="Не оплачен">Не оплачен</option>
                            <option value="Частично оплачен">Частично оплачен</option>
                            <option value="Оплачен">Оплачен</option>
                        </select>
                        <input type="text" class="form-control" id="searchInput" placeholder="Поиск по клиенту...">
                    </div>

                    <div class="orders-list" id="ordersList"></div>
                </div>
            </div>

            <!-- Склад -->
            <div id="warehouse" class="tab-panel">
                <div class="container">
                    <div class="section-header">
                        <h2>Склад</h2>
                        <button class="btn btn--primary" id="addWarehouseBtn">+ Добавить товар</button>
                    </div>
                    
                    <div class="warehouse-list" id="warehouseList"></div>
                </div>
            </div>

            <!-- Аналитика -->
            <div id="analytics" class="tab-panel">
                <div class="container">
                    <div class="section-header">
                        <h2>Аналитика</h2>
                    </div>
                    
                    <div class="analytics-grid">
                        <div class="card">
                            <div class="card__body">
                                <h3>Статистика по статусам</h3>
                                <div id="statusStats"></div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card__body">
                                <h3>Топ товары</h3>
                                <div id="topProducts"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Настройки -->
            <div id="settings" class="tab-panel">
                <div class="container">
                    <div class="section-header">
                        <h2>Настройки</h2>
                    </div>
                    
                    <div class="settings-form">
                        <div class="form-group">
                            <label class="form-label">Курс юаня к тенге</label>
                            <input type="number" class="form-control" id="exchangeRate" min="1" step="0.1">
                        </div>
                        
                        <button class="btn btn--primary" id="saveSettings">Сохранить</button>
                        <button class="btn btn--outline" id="clearData">Очистить данные</button>
                    </div>
                </div>
            </div>
        </div>

        <!-- Навигация -->
        <nav class="nav-tabs">
            <button class="nav-tab active" data-tab="dashboard">
                <div class="nav-icon">🏠</div>
                <div class="nav-label">Главная</div>
            </button>
            <button class="nav-tab" data-tab="orders">
                <div class="nav-icon">📋</div>
                <div class="nav-label">Заказы</div>
            </button>
            <button class="nav-tab" data-tab="warehouse">
                <div class="nav-icon">📦</div>
                <div class="nav-label">Склад</div>
            </button>
            <button class="nav-tab" data-tab="analytics">
                <div class="nav-icon">📊</div>
                <div class="nav-label">Аналитика</div>
            </button>
            <button class="nav-tab" data-tab="settings">
                <div class="nav-icon">⚙️</div>
                <div class="nav-label">Настройки</div>
            </button>
        </nav>
    </div>

    <!-- Модальное окно нового заказа -->
    <div class="modal" id="orderModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Новый заказ</h3>
                <button class="modal-close" id="closeModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="orderForm">
                    <div class="form-group">
                        <label class="form-label">Клиент</label>
                        <input type="text" class="form-control" id="clientName" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Телефон</label>
                        <input type="tel" class="form-control" id="clientPhone" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Город</label>
                        <select class="form-control" id="clientCity" required>
                            <option value="">Выберите город</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Товар</label>
                        <select class="form-control" id="productSelect" required>
                            <option value="">Выберите товар</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Размер</label>
                        <select class="form-control" id="sizeSelect" required>
                            <option value="">Выберите размер</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Цвет</label>
                        <select class="form-control" id="colorSelect" required>
                            <option value="">Выберите цвет</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Количество</label>
                        <input type="number" class="form-control" id="quantity" min="1" value="1" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Итоговая сумма (₸)</label>
                        <input type="number" class="form-control" id="totalAmount" min="0" step="1" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Предоплата (₸)</label>
                        <input type="number" class="form-control" id="prepayment" min="0" step="1" value="0">
                    </div>

                    <div class="calc-info">
                        <div class="calc-row">
                            <span>Себестоимость:</span>
                            <strong id="costDisplay">0 ₸</strong>
                        </div>
                        <div class="calc-row">
                            <span>Прибыль:</span>
                            <strong id="profitDisplay">0 ₸</strong>
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn--outline" id="cancelOrder">Отмена</button>
                        <button type="submit" class="btn btn--primary">Создать заказ</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- Модальное окно редактирования заказа -->
    <div class="modal" id="editOrderModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Редактировать заказ</h3>
                <button class="modal-close" id="closeEditModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="editOrderForm">
                    <div class="form-group">
                        <label class="form-label">Статус оплаты</label>
                        <select class="form-control" id="editPaymentStatus">
                            <option value="Не оплачен">Не оплачен</option>
                            <option value="Частично оплачен">Частично оплачен</option>
                            <option value="Оплачен">Оплачен</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Статус доставки</label>
                        <select class="form-control" id="editDeliveryStatus">
                            <option value="Новый">Новый</option>
                            <option value="В обработке">В обработке</option>
                            <option value="Отправлен">Отправлен</option>
                            <option value="Доставлен">Доставлен</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Доплата (₸)</label>
                        <input type="number" class="form-control" id="editPostpayment" min="0" step="1">
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn--outline" id="cancelEdit">Отмена</button>
                        <button type="submit" class="btn btn--primary">Сохранить</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- Модальное окно добавления товара на склад -->
    <div class="modal" id="warehouseModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Добавить товар на склад</h3>
                <button class="modal-close" id="closeWarehouseModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="warehouseForm">
                    <div class="form-group">
                        <label class="form-label">Товар</label>
                        <input type="text" class="form-control" id="warehouseProduct" placeholder="Название товара" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Размер</label>
                        <input type="text" class="form-control" id="warehouseSize" placeholder="Размер" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Цвет</label>
                        <input type="text" class="form-control" id="warehouseColor" placeholder="Цвет" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Количество</label>
                        <input type="number" class="form-control" id="warehouseStock" min="1" value="1" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Себестоимость в юанях</label>
                        <input type="number" class="form-control" id="warehouseCostYuan" min="0" step="0.01" required>
                    </div>

                    <div class="calc-info">
                        <div class="calc-row">
                            <span>Себестоимость в тенге:</span>
                            <strong id="warehouseCostTenge">0 ₸</strong>
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn--outline" id="cancelWarehouse">Отмена</button>
                        <button type="submit" class="btn btn--primary">Добавить товар</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- Модальное окно редактирования товара на складе -->
    <div class="modal" id="editWarehouseModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Редактировать товар</h3>
                <button class="modal-close" id="closeEditWarehouseModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="editWarehouseForm">
                    <div class="form-group">
                        <label class="form-label">Товар</label>
                        <input type="text" class="form-control" id="editWarehouseProduct" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Размер</label>
                        <input type="text" class="form-control" id="editWarehouseSize" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Цвет</label>
                        <input type="text" class="form-control" id="editWarehouseColor" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Количество</label>
                        <input type="number" class="form-control" id="editWarehouseStock" min="0" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Себестоимость в юанях</label>
                        <input type="number" class="form-control" id="editWarehouseCostYuan" min="0" step="0.01" required>
                    </div>

                    <div class="calc-info">
                        <div class="calc-row">
                            <span>Себестоимость в тенге:</span>
                            <strong id="editWarehouseCostTenge">0 ₸</strong>
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn--outline" id="cancelEditWarehouse">Отмена</button>
                        <button type="submit" class="btn btn--primary">Сохранить изменения</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <script src="app.js"></script>
</body>
</html>'''

# Сохраняю HTML файл
with open('index_fixed.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print("✅ Создан исправленный HTML файл (index_fixed.html)")
print("📝 Основные изменения:")
print("   - Форма добавления товара теперь использует input поля")
print("   - Добавлена форма редактирования товаров на складе")
print("   - Исправлены ID элементов для корректной работы JS")